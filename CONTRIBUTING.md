...
- **Before submit**: Run `uv run pytest`, `uv run ruff check src tests scripts`, `uv run ruff format src tests scripts` (or `uv run pre-commit run --all-files`). Optionally: Pylint, mypy.
