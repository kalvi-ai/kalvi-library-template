...
| **Indexing** | `.cursorignore` | Excludes paths from codebase indexing for a faster, leaner index. |

## Hooks

Hooks run custom scripts at specific points during the agent’s execution. Config: `.cursor/hooks.json` in the project or `~/.cursor/hooks.json` globally.
...
- **Codebase indexing**: Cursor indexes the repo for semantic search. `.cursorignore` in the repo root excludes venv, caches, and build output so indexing stays fast. Edit in Settings → Indexing & Docs → “Ignore Files in .cursorignore”.
