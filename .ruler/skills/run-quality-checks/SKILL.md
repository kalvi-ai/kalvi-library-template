...
Run in order: `uv run pytest` → `uv run ruff check src tests scripts` → `uv run ruff format --check src tests scripts` → `uv run pylint src/kalvi_library_template scripts` → `uv run mypy src/kalvi_library_template scripts` → `uv run pre-commit run --all-files`. Or just `uv run pre-commit run --all-files` (runs YAML/JSON/TOML/Markdown/typos + Ruff); CI runs the same.
